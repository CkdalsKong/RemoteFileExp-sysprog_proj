...
void server() {
	...
	while ((recv_size = read(clnt_sock, message, BUF_SIZE - 1)) > 0) {
		...
		if (!strcmp(message, "homedir"))
			...
			printf("%s\n", modeinfo[1]);
			stat(modeinfo[0], &s_info);
			if (S_ISDIR(s_info.st_mode)) {
				if (!strcmp(modeinfo[1], "Enter")) {
					sendDirectoryInfo(clnt_sock, modeinfo[0]);
				}
			}
			else {
				if (!strcmp(modeinfo[1], "download")) {
					sendFile(clnt_sock);
				}
			}
		}
...
void sendFile(int sock) {
	...
	fp = fopen(modeinfo[0], "rb");
	if (fp == NULL)
		error_handling("fopen() error");
	stat(modeinfo[0], &info);
	file_size = info.st_size;
	
	send(sock, modeinfo[0], BUF_SIZE, 0);
	send(sock, &file_size, sizeof(long), 0);
	
	while ((read_len = fread(buf, 1, BUF_SIZE, fp)) > 0) {
		if (send(sock, buf, read_len, 0) != read_len) 
...
void parseMessage(char *message) {
	MALLOC(modeinfo[0], 1024 * sizeof(char));
	MALLOC(modeinfo[1], 1024 * sizeof(char));
